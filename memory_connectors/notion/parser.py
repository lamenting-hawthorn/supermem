...
class NotionParser:
    ...
    def _parse_directory_export(self, export_dir: Path) -> ParsedNotionData:
        ...
        workspace = NotionWorkspace(
            pages=pages, databases=databases, export_date=datetime.now()
        )
    ...
    def _parse_markdown_blocks(self, content: str) -> List[NotionBlock]:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("# "):
                ...
                block_content = line[2:].strip()
            elif line.startswith("1. ") or any(
                line.startswith(f"{j}. ") for j in range(2, 100)
            ):
            ...
            block = NotionBlock(
                id=f"block_{i}", type=block_type, content=block_content, children=[]
            )
            blocks.append(block)
