...
class NotionConnector(BaseMemoryConnector):
    ...
    def organize_data(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(
            f"🗂️ Organizing {extracted_data['total_pages']} pages across {len(topics)} topics"
        )
    ...
    def generate_memory_files(self, organized_data: Dict[str, Any]) -> None:
        ...
        # Index all generated markdown files into supermem storage
        try:
        ...
        if "## Available Knowledge Sources" in existing_content:
            ...
            for line in lines:
                ...
                if line == "## Available Knowledge Sources":
                    ...
                    in_knowledge_section
                    and line.startswith("##")
                    and "Available Knowledge Sources" not in line
                ):
                    ...
                    if not added_notion:
                        ...
                        new_lines.insert(
                            -1,
                            f"Complete Notion workspace ({sum(len(pages) for pages in topic_pages.values())} pages across {len(topic_pages)} topics):",
                        )
                        new_lines.insert(
                            -1,
...
"""
        ...
        sorted_topics = sorted(
            topic_pages.items(), key=lambda x: len(x[1]), reverse=True
        )

        for topic, pages in sorted_topics:
            topic_title = topic.replace("-", " ").title()
            index_content += (
                f"- **[[topics/{topic}.md|{topic_title}]]** ({len(pages)} pages)\\n"
            )
...
```"""
    ...
    def _generate_topic_file(self, topic, pages, topics_dir) -> None:
        ...
        # Get date range
        dates = [
            page.last_edited_time or page.created_time
            for page in pages
            if page.last_edited_time or page.created_time
        ]
        date_range = ""
        if dates:
...
"""
        ...
        sorted_pages = sorted(
            pages,
            key=lambda x: x.last_edited_time or x.created_time or datetime.min,
            reverse=True,
        )
        ...
        for i, page in enumerate(sorted_pages):
            ...
            content += (
                f"- **[[../pages/{filename}|{title}]]** ({len(page.blocks)} blocks"
            )
            if date_str:
                content += f", {date_str}"
            content += ")\\n"
...
"""
    ...
    def _generate_page_file(
        self, page: NotionPage, index: int, topic: str, pages_dir: Path
    ) -> None:
        ...
        # Format timestamps
        created = (
            page.created_time.strftime("%Y-%m-%d %H:%M:%S")
            if page.created_time
            else "Unknown"
        )
        updated = (
            page.last_edited_time.strftime("%Y-%m-%d %H:%M:%S")
            if page.last_edited_time
            else created
        )
