...
class GitHubLiveConnector(BaseMemoryConnector):
    ...
    def _fetch_repository_data(self, repo_name: str) -> Dict[str, Any]:
        ...
        print(
            f"✅ Repository found: {repo_info.get('full_name')} ({'private' if repo_info.get('private') else 'public'})"
        )
        default_branch = repo_info.get("default_branch", "main")
    ...
    def _fetch_repository_structure(
        self, repo_name: str, branch: str = "main"
    ) -> List[Dict[str, Any]]:
        """Fetch repository structure and important files."""
        files = []
        max_files_per_repo = min(
            50, (self.max_items or 100) // 2
        )  # Reserve half for other content

        try:
            # Get root directory contents
            contents = self._github_api_call(
                f"/repos/{repo_name}/contents?ref={branch}"
            )
            ...
            for item in contents:
                ...
                if item_type == "file":
                    ...
                    dir_files = self._fetch_directory_contents(
                        repo_name,
                        item_path,
                        branch,
                        max_files_per_repo - files_fetched,
                        depth=1,
                    )
    ...
    def _fetch_directory_contents(
        ...
        try:
            contents = self._github_api_call(
                f"/repos/{repo_name}/contents/{dir_path}?ref={branch}"
            )
    ...
    def _fetch_issues(self, repo_name: str) -> List[Dict[str, Any]]:
        ...
        for state in ["open", "closed"]:
            issues_data = self._github_api_call(
                    ...
                    issues.append(
                        {
                            ...
                            "author": (
                                issue["user"]["login"] if issue.get("user") else None
                            ),
                        }
                    )
    ...
    def _fetch_pull_requests(self, repo_name: str) -> List[Dict[str, Any]]:
        ...
        max_per_request = min(25, (self.max_items or 100) // 4)
        for state in ["open", "closed"]:
            prs_data = self._github_api_call(
    ...
    def generate_memory_files(self, organized_data: Dict[str, Any]) -> None:
        ...
        for topic_name, topic_data in organized_data["topics"].items():
            ...
            for item in topic_data["items"]:
                item_file = (
                    items_dir
                    / f"item_{item_counter}-{self._sanitize_filename(item['title'])}.md"
                )
...
"""
        ...
        for topic_name, topic_data in organized_data["topics"].items():
            item_count = len(topic_data["items"])
            content += (
                f"- **[[topics/{topic_name}.md|{topic_name}]]** ({item_count} items)\n"
            )
...
"""
    ...
    def _generate_repository_file(
        self, repo_file: Path, topic_data: Dict[str, Any]
    ) -> None:
...
"""
        ...
        for content_type, count in content_types.items():
            content += (
                f"- **{content_type.replace('_', ' ').title()}:** {count} items\n"
            )
    ...
    def _generate_item_file(
        self, item_file: Path, item: Dict[str, Any], repo_name: str
    ) -> None:
...
"""
    ...
    def _update_user_profile(
        self, memory_dir: Path, organized_data: Dict[str, Any]
    ) -> None:
        ...
        if "### GitHub Repositories" in existing_content:
            ...
            for line in lines:
                if line.startswith("### GitHub Repositories"):
                    ...
                    new_lines.append(
                        f"GitHub repositories ({organized_data['total_items']} items across {len(organized_data['repositories'])} repositories):"
                    )
                    ...
                    for topic_name, topic_data in organized_data["topics"].items():
                        ...
                        item_count = len(topic_data["items"])
                        new_lines.append(
                            f"- **[[entities/github-repositories/repositories/{topic_name}.md|{repo_name}]]** ({item_count} items)"
                        )
                    new_lines.append("")
                    new_lines.append(line)
                elif not in_github_section or not line.startswith(
            ...
            if existing_content:
                ...
                if "### Search Strategy" in existing_content:
                    updated_content = existing_content.replace(
                        "### Search Strategy", github_section + "\n### Search Strategy"
                    )
                else:
