...
class NuclinoConnector(BaseMemoryConnector):
    ...
    def organize_data(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(
            f"🗂️ Organizing {extracted_data['total_items']} items across {len(topics)} topics"
        )
            ...
            for line in lines:
                ...
                if line == "## Available Knowledge Sources":
                    ...
                    if not added_nuclino:
                        ...
                        new_lines.insert(
                            -1,
                            f"Complete Nuclino workspace ({sum(len(items) for items in topic_items.values())} items across {len(topic_items)} topics):",
                        )
                        new_lines.insert(
                            -1,
...
"""
        ...
        sorted_topics = sorted(
            topic_items.items(), key=lambda x: len(x[1]), reverse=True
        )

        for topic, items in sorted_topics:
            topic_title = topic.replace("-", " ").title()
            index_content += (
                f"- **[[topics/{topic}.md|{topic_title}]]** ({len(items)} items)\\n"
            )
...
```"""
    ...
    def _generate_cluster_file(
        self, cluster: NuclinoCluster, clusters_dir: Path
    ) -> None:
...
"""
    ...
    def _generate_topic_file(self, topic, items, topics_dir) -> None:
        ...
        # Get date range
        dates = [
            item.modified_time or item.created_time
            for item in items
            if item.modified_time or item.created_time
        ]
        date_range = ""
        if dates:
...
"""
    ...
    def _generate_item_file(
        self, item: NuclinoItem, index: int, topic: str, items_dir: Path
    ) -> None:
...
"""
        ...
        if item.attachments:
            ...
            for attachment in item.attachments:
                content += (
                    f"- **{attachment.filename}** ({attachment.original_path})\\n"
                )
            content += "\\n---\\n\\n"
