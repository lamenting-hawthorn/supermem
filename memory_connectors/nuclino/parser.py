...
class NuclinoParser:
    ...
    def _parse_directory_export(self, export_dir: Path) -> ParsedNuclinoData:
        ...
        # Create workspace
        workspace_name = (
            export_dir.name if export_dir.name != "temp" else "Nuclino Workspace"
        )
            ...
            if folder_path.exists() and folder_path.is_dir():
                ...
                for file_path in folder_path.rglob("*"):
                    if file_path.is_file():
                        ...
                        self.attachment_map[attachment.original_path] = (
    ...
    def _parse_markdown_item(
        self, file_path: Path, export_dir: Path
    ) -> Optional[NuclinoItem]:
        ...
        try:
            ...
            title = self._extract_title_from_content(
                content
            ) or self._extract_title_from_filename(file_path)
    ...
    def _extract_cluster_from_path(
        self, file_path: Path, export_dir: Path
    ) -> Optional[str]:
    ...
    def _extract_attachments_from_content(
        self, content: str
    ) -> List[NuclinoAttachment]:
        ...
        for match in re.finditer(link_pattern, content):
            ...
            if (
                ...
                and path in self.attachment_map
            ):
                attachments.append(
                    NuclinoAttachment(
                        filename=Path(path).name,
    ...
    def _organize_into_clusters(self, items: List[NuclinoItem]) -> List[NuclinoCluster]:
        ...
        for item in items:
            ...
            if cluster_name not in cluster_map:
                cluster_map[cluster_name] = NuclinoCluster(name=cluster_name, items=[])
    ...
    def _organize_by_topics(
        ...
        for item in items:
            ...
            text_to_analyze = (
                f"{item.title} {item.cluster_name or ''} {item.content[:500]}".lower()
            )
