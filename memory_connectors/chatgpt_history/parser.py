...
class ChatGPTParser:
    ...
    def parse_export(self, export_path: str) -> Dict[str, any]:
        ...
        for i, conversation_data in enumerate(raw_conversations):
            ...
            if not global_user_profile and parsed.user_profile:
                global_user_profile = self._extract_user_profile(
                    parsed.user_profile, parsed.user_instructions
                )
        ...
        return {
            ...
            + 2,  # +2 for conversations.json and chat.html
    ...
    def _parse_conversation(
        self, conversation: ChatGPTConversation, conv_id: str
    ) -> ParsedConversation:
        ...
        for node_id in message_order:
            ...
            if (
                message.author.get("role") == "system"
                and not self.options.include_system_messages
            ):
                continue
            ...
            parsed_message = ParsedMessage(
                ...
                content=content,
                timestamp=(
                    datetime.fromtimestamp(message.create_time)
                ...
                metadata=(
                    message.metadata
                    if self.options.include_metadata and message.metadata
                    else {}
                ),
            )
    ...
    def _extract_user_profile(
        self, user_profile_text: Optional[str], user_instructions_text: Optional[str]
    ) -> UserProfile:
        ...
        if user_instructions_text:
            ...
            instructions_match = re.search(
                r"```([^`]+)```", user_instructions_text, re.DOTALL
            )
