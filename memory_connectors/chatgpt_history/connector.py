    ...
    def generate_memory_files(self, organized_data: Dict[str, Any]) -> None:
        ...
        for topic, conversations in topic_conversations.items():
            ...
            for i, conversation in enumerate(conversations):
                self._generate_conversation_file(
                    conversation, i, topic, conversations_dir
                )
                total_files += 1
...
"""
        ...
        sorted_topics = sorted(
            topic_conversations.items(), key=lambda x: len(x[1]), reverse=True
        )
...
"""
        ...
        sorted_conversations = sorted(
            conversations, key=lambda x: x.created_at or x.updated_at, reverse=True
        )
...
"""
    ...
    def _generate_conversation_file(
        self, conversation, index, topic, conversations_dir
    ) -> None:
...
"""
        ...
        for message in conversation.messages:
            ...
            if (
                ...
                and "model_slug" in message.metadata
            ):
