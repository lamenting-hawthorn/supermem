...
class ChatGPTEmbeddingConnector(BaseMemoryConnector):
    ...
    def __init__(self, output_path: str, **kwargs):
        ...
        self.lmstudio_base_url = kwargs.get(
        ...
        self.distance_threshold = kwargs.get(
    ...
    def _load_embedding_model(self):
        ...
        if self.model_name.startswith("lmstudio:"):
            ...
            try:
                ...
                raise ImportError(
                    f"Missing dependencies for LM Studio embedding: {e}. Install with: pip install requests scikit-learn"
                )
        ...
        raise ValueError(
            f"Unsupported embedding model: {self.model_name}. Supported models: tfidf, lmstudio:model-name"
        )
        ...
        try:
            ...
            if response.status_code != 200:
                raise ConnectionError(
                    f"LM Studio server not responding (HTTP {response.status_code})"
                )
            ...
            if self._start_lmstudio_server(model_id):
                ...
                try:
                    response = requests.get(
                        f"{self.lmstudio_base_url}/v1/models", timeout=10
                    )
                    if response.status_code == 200:
                        models = response.json()
                        available_models = [
                        ...
                        if model_id not in available_models:
                            ...
                            print(f"💡 Please load '{model_id}' in LM Studio first")
                            raise ValueError(
                                f"Model '{model_id}' not available in LM Studio"
                            )
                        return  # Success
                except requests.exceptions.ConnectionError:
                    pass
    ...
    def _start_lmstudio_server(self, model_id: str) -> bool:
        ...
        try:
            ...
            try:
                subprocess.run(
                    ["lms", "get", model_id, "--always-show-all-results"], check=False
                )
            except Exception:
            ...
            while time.time() - start_time < max_wait_seconds:
                try:
                    response = requests.get(
                        f"{self.lmstudio_base_url}/v1/models", timeout=2
                    )
                    if response.status_code == 200:
                        models = response.json()
    ...
    def _call_lmstudio_embeddings(self, texts: list) -> list:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            payload = {"model": self.model, "input": batch_texts}  # model ID
    ...
    def _create_conversation_text(self, conversation) -> str:
        ...
        if conversation.messages:
            # First user message
            first_msg = next(
                (msg for msg in conversation.messages if msg.role == "user"), None
            )
    ...
    def _discover_clusters(self, embeddings):
        ...
        clustering = AgglomerativeClustering(
            n_clusters=None, distance_threshold=self.distance_threshold, linkage="ward"
        )
    ...
    def _extract_cluster_themes(
        self, conversations: List, cluster_labels
    ) -> Dict[int, List[str]]:
        ...
        for label in unique_labels:
            ...
            try:
                vectorizer = TfidfVectorizer(
                    max_features=100, stop_words="english", ngram_range=(1, 2), min_df=2
                )
    ...
    def _heuristic_category_naming(self, themes: List[str]) -> str:
        ...
        if any(word in theme_text for word in ["dria", "firstbatch", "network"]):
            ...
            word in theme_text for word in ["llm", "language model", "gpt", "claude"]
        ):
            return "LLMs"
        elif any(
            word in theme_text
            for word in ["product", "strategy", "business", "roadmap"]
        ):
            return "Product Strategy"
        elif any(
            word in theme_text
            for word in ["python", "code", "programming", "development"]
        ):
            return "Programming"
        elif any(
            word in theme_text for word in ["data", "analytics", "machine learning"]
        ):
            return "Data Science"
            ...
            word in theme_text for word in ["technical", "architecture", "system"]
        ):
    ...
    def organize_data(self, extracted_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(conversations) < self.min_cluster_size:
            print(
                f"⚠️ Too few conversations ({len(conversations)}) for clustering. Using single category."
            )
            return {
        ...
        for i, (conversation, cluster_id) in enumerate(
            zip(conversations, cluster_labels)
        ):
            # Skip small clusters
            import numpy as np
        ...
        for category, convs in topic_conversations.items():
            display_name = cluster_info.get(category, {}).get(
    ...
    def generate_memory_files(self, organized_data: Dict[str, Any]) -> None:
        ...
        print(
            f"✅ Generated {total_files} memory files using AI-powered categorization"
        )

    def _generate_user_md(
        self,
        user_profile: Dict[str, Any],
        topic_conversations: Dict[str, List],
...
"""
    ...
    def _generate_conversation_file(
        self, conv_file: Path, conversation, topic_slug: str
    ):
