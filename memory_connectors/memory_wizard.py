...
class MemoryWizard:
        ...
        while True:
            try:
                ...
                if 0 <= choice_idx < len(connector_list):
                    ...
                    print(
                        f"❌ Please enter a number between 1 and {len(connector_list)}"
                    )
    ...
    def _get_export_inputs(self, info: Dict):
        ...
        print(
            f"You need to provide the path to your {info['name'].lower()} export file."
        )
        ...
        while True:
            ...
            if path_obj.is_file():
                if path_obj.suffix.lower() not in info["formats"] and info[
                    "formats"
                ] != [".zip", ".json"]:
                    print(
                        f"⚠️ Warning: File extension {path_obj.suffix} not in expected formats {info['formats']}"
                    )
    ...
    def _get_live_inputs(self, info: Dict):
        ...
        source = (
            input("Enter source (repository, folder ID, or URL): ").strip().strip("\"'")
        )
        if not source:
    ...
    def _get_auth_token(self):
        ...
        if self.selected_connector == "github":
            ...
            print(
                "You need a Personal Access Token from: https://github.com/settings/tokens"
            )
        ...
        if self.selected_connector == "github":
            skip_token = (
                input("Skip token for public repositories only? (y/n): ")
                .strip()
                .lower()
            )
            if skip_token == "y":
        ...
        while True:
            try:
                token = getpass.getpass(
                    "Enter your access token (hidden input): "
                ).strip()
                if token:
    ...
    def _get_chatgpt_options(self):
        ...
        while True:
            ...
            if choice in ["1", "2"]:
                ...
                print(
                    f"✅ Selected: {'Keyword-based' if method == 'keyword' else 'AI-powered'}"
                )
                break
        ...
        if method == "keyword":
            ...
            edit_keywords = (
                input("Do you want to customize the keyword categories? (y/N): ")
                .strip()
                .lower()
            )
            if edit_keywords == "y":
            ...
            print(
                "    └─ Requirements: LM Studio with text-embedding-nomic-embed-text-v1.5"
            )
            print()
            ...
            while True:
                ...
                if embedding_choice in ["1", "2"]:
                    ...
                    print(
                        f"✅ Selected: {'TF-IDF' if embedding_model == 'tfidf' else 'Nomic Embed (LM Studio)'}"
                    )
                    break
                print("Please enter 1 or 2")
    ...
    def _confirm_and_run(self):
        ...
        if "token" in self.connector_params:
            token_status = (
                "✅ Provided"
                if self.connector_params["token"]
                else "⚠️ None (public only)"
            )
        ...
        if self.selected_connector == "github":
            ...
            if method == "keyword" and self.connector_params.get("edit_keywords"):
                ...
                print(
                    f"Embedding model: {'TF-IDF' if embedding == 'tfidf' else 'Nomic Embed (LM Studio)'}"
                )
    ...
    def _run_connector(self) -> bool:
        ...
        try:
            ...
            cmd = [
                sys.executable,
                "-m",
                "memory_connectors.memory_connect",
            ...
            # Add ChatGPT-specific options
                ...
                if "embedding_model" in self.connector_params:
                    cmd.extend(
                        ["--embedding-model", self.connector_params["embedding_model"]]
                    )
    ...
    def _show_failure(self):
        ...
        if info["type"] == "export":
            ...
            if self.selected_connector == "github":
                ...
                print(
                    "• Ensure your token has 'drive.readonly' scope (not 'drive.apps.readonly')"
                )
