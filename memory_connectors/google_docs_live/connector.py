...
class GoogleDocsLiveConnector(BaseMemoryConnector):
    ...
    def extract_data(self, source_path: str) -> Dict[str, Any]:
        ...
        all_data = {"folder_id": folder_id, "documents": [], "total_items": 0}
        ...
        try:
            ...
            for doc in documents:
                try:
                    ...
                    print(
                        f"⚠️ Error fetching document {doc.get('name', 'unknown')}: {e}"
                    )
                    continue
    ...
    def _parse_folder_id(self, source_path: str) -> str:
        ...
        if "drive.google.com" in source_path:
            ...
            if "/folders/" in source_path:
                ...
                folder_id = (
                    source_path.split("/folders/")[-1].split("?")[0].split("#")[0]
                )
                return folder_id
    ...
    def _get_folder_info(self, folder_id: str) -> Optional[Dict]:
        ...
        params = {"fields": "id,name,description,createdTime,modifiedTime,owners"}
    ...
    def _get_documents_in_folder(self, folder_id: str) -> List[Dict]:
        ...
        params = {
            ...
            "pageSize": min(
                100, (self.max_items or 50) * 2
            ),  # Get more than max in case some fail
        }
    ...
    def _categorize_documents_by_topics(
    ...
    def generate_memory_files(self, organized_data: Dict[str, Any]) -> None:
        ...
        for topic_name, topic_docs in organized_data["topics"].items():
            ...
            for doc in topic_docs:
                doc_file = (
                    documents_dir
                    / f"doc_{doc_counter}-{self._sanitize_filename(doc['name'])}.md"
                )
...
"""
    ...
    def _generate_topic_file(
        self, topic_file: Path, topic_name: str, docs: List[Dict], start_index: int
    ) -> None:
