...
def choose_chatgpt_method() -> tuple:
    ...
    for i, method in enumerate(methods, 1):
        ...
        if method == "ai":
            print(
                f"    └─ Note: Requires additional dependencies (transformers, torch, scikit-learn)"
            )
        print()
...
def add_keyword_category(keywords_dict):
    ...
    keywords_list = [
        kw.strip().lower() for kw in keywords_input.split(",") if kw.strip()
    ]
    ...
    try:
        ...
        if 0 <= choice < len(categories):
            ...
            if new_keywords_input:
                new_keywords = [
                    kw.strip().lower()
                    for kw in new_keywords_input.split(",")
                    if kw.strip()
                ]
...
def delete_keyword_category(keywords_dict):
    ...
    try:
        ...
        if 0 <= choice < len(categories):
            ...
            confirm = (
                input(f"Are you sure you want to delete '{category}'? [y/N]: ")
                .strip()
                .lower()
            )
            if confirm == "y":
...
Examples:
  ...
  python memory_connect.py --list
    ...
    parser.add_argument("source", nargs="?", help="Path to source data")
    ...
    parser.add_argument(
        "--max-items", "-m", type=int, help="Maximum number of items to process"
    )
    ...
    parser.add_argument(
        "--list", "-l", action="store_true", help="List available connectors"
    )
    ...
    if args.connector == "chatgpt":
        if args.method:
            method = args.method
            print(
                f"✅ Using {CHATGPT_METHODS[method]['name']} (specified via --method)"
            )
    ...
    if not is_live_connector:
        source_ext = (
            source_path.suffix.lower() if source_path.is_file() else "directory"
        )
        supported_formats = (
            connector_instance.supported_formats + ["directory"]
            if args.connector == "chatgpt"
            else connector_instance.supported_formats
        )
        if source_ext not in supported_formats:
            print(
                f"❌ Unsupported format '{source_ext}' for {args.connector} connector"
            )
        ...
        if token is None:  # No --token argument provided at all
            ...
            try:
                ...
                if args.connector == "github":
                    ...
                    print()
                    print(
                        "   Press Enter to proceed without token (public API, rate limited)"
                    )
                    token = getpass.getpass(
                        "Enter your GitHub token (or press Enter): "
                    )
                ...
                print(
                    "⚠️ No authentication token provided - proceeding with public API (rate limited)"
                )
                token = None
    ...
    try:
        ...
        if is_live_connector:
            if args.connector == "github":
                ...
                connector = connector_class(
                    args.output, access_token=token, max_items=args.max_items
                )
            else:
                connector = connector_class(
                    args.output, token=token, max_items=args.max_items
                )
        else:
