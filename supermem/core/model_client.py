...
class BaseModelClient(ABC):
    ...
    - chat_completion() MUST raise LLMProviderError (from supermem.errors) on failure.
    - from_env() MUST read SUPERMEM_LLM_PROVIDER and return the matching subclass.
    ...
    def from_env(cls) -> "BaseModelClient":
        """
        Factory: reads SUPERMEM_LLM_PROVIDER env var, returns the matching subclass.

        Supported values: openrouter | ollama | vllm | claude | lmstudio
        Defaults to openrouter.
        """
        from supermem.model.base import get_client_for_provider  # noqa: PLC0415

        provider = os.getenv("SUPERMEM_LLM_PROVIDER", "openrouter").strip().lower()
