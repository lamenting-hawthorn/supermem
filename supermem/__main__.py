...
Commands:
  ...
  supermem serve --worker   Start MCP server + Worker HTTP API on :37777
  supermem chat             Interactive terminal REPL
  supermem backup           Archive vault + SQLite to timestamped .tar.gz
  supermem restore <file>   Restore vault + SQLite from archive
  supermem connect <type> <source>  Run a memory connector

Usage examples:
  supermem serve
  supermem serve --worker
  supermem chat
  supermem backup
  supermem backup --output /tmp/my_backup.tar.gz
  supermem restore /tmp/supermem_backup_20260101_120000.tar.gz
  supermem connect chatgpt ~/Downloads/chatgpt-export.zip
...
# Ensure project root is on path when running as `python -m supermem`
...
def _worker_port() -> int:
    try:
        from supermem.config import SUPERMEM_WORKER_PORT

        return SUPERMEM_WORKER_PORT
    except Exception:
        return 37777
...
def cmd_backup(args: argparse.Namespace) -> None:
    ...
    output = (
        Path(args.output)
        if args.output
        else Path.cwd() / f"supermem_backup_{timestamp}.tar.gz"
    )
    ...
    with tarfile.open(output, "w:gz") as tar:
        if SUPERMEM_VAULT_PATH.exists():
            for md in SUPERMEM_VAULT_PATH.rglob("*.md"):
                tar.add(str(md), arcname=f"vault/{md.relative_to(SUPERMEM_VAULT_PATH)}")
                print(f"  + {md.relative_to(SUPERMEM_VAULT_PATH)}", flush=True)
        if SUPERMEM_DB_PATH.exists():
            tar.add(str(SUPERMEM_DB_PATH), arcname="supermem.db")
            print(
                f"  + supermem.db ({SUPERMEM_DB_PATH.stat().st_size // 1024} KB)",
                flush=True,
            )
...
def cmd_restore(args: argparse.Namespace) -> None:
    ...
    with tarfile.open(archive, "r:gz") as tar:
        for member in tar.getmembers():
            ...
            elif member.name == "supermem.db":
                SUPERMEM_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
                f = tar.extractfile(member)
                if f:
                    SUPERMEM_DB_PATH.write_bytes(f.read())
                    print("  restored: supermem.db", flush=True)
...
def main() -> None:
    ...
    p_backup.add_argument(
        "--output", "-o", help="Output path (default: cwd/supermem_backup_<ts>.tar.gz)"
    )
